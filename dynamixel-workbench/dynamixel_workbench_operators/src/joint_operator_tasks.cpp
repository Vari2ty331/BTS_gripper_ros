...
JointOperator::JointOperator(std::string tasks)
  :node_handle_(""),
   priv_node_handle_("~"),
   is_loop_(false)
{
  ...
  joint_trajectory_pub_ = node_handle_.advertise<trajectory_msgs::JointTrajectory>("joint_trajectory", 100);
  move_command_server_ = node_handle_.advertiseService(tasks, &JointOperator::moveCommandMsgCallback, this);
...
int main(int argc, char **argv)
{
  ...
  JointOperator fold_operator(std::string("finger_fold"));
  JointOperator unfold_operator(std::string("finger_unfold"));
  JointOperator grip_operator(std::string("grip_to_size"));
  JointOperator initialize_operator(std::string("initialize_to_max"));
